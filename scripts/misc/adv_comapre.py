...
# New function: compare_clean_vs_adv
def compare_clean_vs_adv(clean_npz, adv_npz, output_dir):
    """Compare latent PCA/t-SNE spaces of clean vs adversarial MNIST for all 8 labels."""
    clean_data = np.load(clean_npz)
    adv_data = np.load(adv_npz)

    clean_latents, clean_labels = clean_data["latents"], clean_data["labels"]
    adv_latents, adv_labels = adv_data["latents"], adv_data["labels"]

    os.makedirs(output_dir, exist_ok=True)

    for label in range(8):
        clean_mask = clean_labels == label
        adv_mask = adv_labels == label
        clean_points = clean_latents[clean_mask]
        adv_points = adv_latents[adv_mask]

        # PCA reduction
        all_points = np.concatenate([clean_points, adv_points], axis=0)
        reduced = PCA(n_components=2).fit_transform(all_points)
        n_clean = len(clean_points)

        plt.figure(figsize=(6, 6))
        plt.scatter(reduced[:n_clean, 0], reduced[:n_clean, 1], s=5, c='blue', label='Clean')
        plt.scatter(reduced[n_clean:, 0], reduced[n_clean:, 1], s=5, c='red', label='Adversarial')
        plt.legend()
        plt.title(f"Latent Comparison for Label {label}")
        plt.xlabel("PC 1")
        plt.ylabel("PC 2")
        plt.savefig(os.path.join(output_dir, f"latent_compare_label_{label}.png"), dpi=300)
        plt.close()

    print(f"Saved comparisons for all 8 labels in {output_dir}")
...
if __name__ == "__main__":
    ...
    clean_npz = os.path.join(output_dir, "mnist_clean_complete.npz")
    adv_npz = os.path.join(output_dir, "adversarial_mnist_all_complete.npz")
    compare_clean_vs_adv(clean_npz, adv_npz, output_dir)
