...
if args.mode == "split":
    ...
    final_w, final_h = 44, 28   # desired final size (width x height) including padding
    pad = 5
    base_w, base_h = final_w - 2 * pad, final_h - 2 * pad  # size before padding
    global_max_w, global_max_h = base_w, base_h
    print(f"Global character size fixed: final (with padding) = {final_w}x{final_h}, base (before padding) = {base_w}x{base_h}")
    ...
    print(f"🔎 Base character size (before padding): width={base_w}, height={base_h}")
    print(f"🖼️ Final saved character image size (with padding): width={final_w}, height={final_h}")
